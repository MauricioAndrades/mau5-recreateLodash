...
describe('#take(array, int)', function() {
  ...
  it('should return int elements from the beginning of the array', function() {
    assert.deepEqual(_.take(['one', 'two'], 0), undefined);
...
describe("#indexOf", function() {
  ...
  it('returns -1 when the second argument is not matched in the array', function() {
    assert.equal(_.indexOf(['one','two'],'three') === -1, true );
  });
...
describe("#size", function() {
  ...
  it('returns null for undefined collections', function() {
    assert.equal(_.size(999), undefined);
  });
...
describe("#forEach", function() {
  ...
  it('goes through each item in a collection', function() {
    var iteratorCount = 0;
    var result = _.forEach([1,2,3,4], function(item){ item + 1; iteratorCount ++; })

    assert.equal(iteratorCount, 4);
  });

  it('goes through each item in an array', function() {
    var result = _.forEach([1,2,3,4], function(item){ item + 1; })
  ...
  it('goes through each property in a collection', function() {
    var iteratorCount = 0;
    var result = _.forEach({a:1,b:2,c:3,d:4}, function(item){ item + 1; iteratorCount ++; })
...
describe("#filter", function() {
  ...
  it('returns an array of all items in a collection where each item returns true when the iteratee is called on ', function() {
    var iteratorCount = 0;
    var result = _.filter([1,2,3,4], function(item){ iteratorCount ++; return item % 2 === 0; })
    var resultTwo = _.filter("pickle", function(item){ return item === 'p'; })
  ...
  it('returns an array of all items in an object where each item returns true when the iteratee is called on it', function() {
    var iteratorCount = 0;
    var result = _.filter({a:1,b:2,c:3,d:4,e:6}, function(item){ iteratorCount ++; return item % 2 === 0; })
...
describe("#reject", function() {
  ...
  it('The opposite of _.filter; this method returns the elements of an ARRAY that predicate does not return truthy for.', function() {
    var iteratorCount = 0;
    var result = _.filter({a:1,b:2,c:3,d:4,e:5}, function(item){ iteratorCount ++; return item % 2 === 0; })
...
describe("#sample", function() {
  ...
  it('Gets a random element or n random elements from a collection.', function() {
